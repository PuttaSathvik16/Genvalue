...
export default function AdminStudentsPage() {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      const headers =
        "ID,Name,Email,Role,AuthProvider,EmailVerified,AccountCreated,LastLogin,Status\n";
      const rows = allUsers
        .map((u) => {
          const status = u.lastLoginAt ? "Active" : "Registered";
          return `"${u.id}","${u.name}","${u.email}","${u.role}","${u.authProvider}","${u.emailVerified}","${u.createdAt}","${u.lastLoginAt ?? ""}","${status}"`;
        })
        .join("\n");
  ...
  const handleConfirmRemove = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(`${studentToRemove.name} has been suspended and removed from GenValue`);
      setStudentToRemove(null);
      setRemovalReason("");
      await loadUsers();
    } catch (err) {
                ...
                users.map((u, index) => {
                  ...
                  return (
                    <tr key={u.id}>
                      ...
                      <td className="py-4 px-4">
                        <span
                          className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold ${status.className}`}
                        >
                          {status.label}
                        </span>
                      </td>
                      ...
                      <td className="py-4 px-4 text-right">
                        {u.role === "STUDENT" ? (
                          <button
                            ...
                            aria-label={`Remove student ${u.name}`}
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-[#6B6558] dark:text-slate-400">
                    This permanently suspends{" "}
                    <span className="font-bold text-[#2A2A28] dark:text-white">
                      {studentToRemove.name}
                    </span>{" "}
                    ({studentToRemove.email}) and deletes their account from the platform.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <form onSubmit={handleConfirmRemove} className="mt-5 space-y-4">
                ...
                <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                  ...
                  <button
                    ...
                    aria-label={`Confirm removal of ${studentToRemove.name}`}
